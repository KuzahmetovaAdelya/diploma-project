...
interface Photo {
  id: string;           // локальный id для UI (photo0, photo1...)
  dbId: number;         // реальный id из таблицы Photos
  src: string;
  liked: boolean;
}
...
export default function ChoosePage() {
    ...
    if (stored) {
      try {
        ...
        if (Array.isArray(raw) && raw.length > 0) {
          const loadedPhotos: Photo[] = raw.map((item: any, index: number) => ({
            id: `photo${index}`,
            dbId: item.id,
            src: `http://localhost:3001/photos/${item.photoName}`,
            liked: false,
          }));
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const dbIds = likedPhotos.map((p) => p.dbId);
    try {
      const response = await fetch('http://localhost:3001/sendPhotos', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email, photos: dbIds }),
      });
